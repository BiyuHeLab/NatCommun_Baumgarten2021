function [series series_pred pred_err fGns fBms] = converging_prediction_log(beta, sigma2, k, nSeries, range_w, range_delta, end_w, end_delta, min_v, doPlot, report)
...
while i_s < nSeries
    ...
    while j+1 <= length(s)
        ...
        % - f_end = end_w * f_min --> s_j - s_min = log(end_w)
        series_maxInd = j;
        series_minInd = j - (k-1);
        s_k = s(series_minInd : series_maxInd);
        
        rangeOK = abs( range(s_k) - log(range_w) ) <= range_delta;
        endOK   = abs( (s_k(end) - min(s_k)) - log(end_w) ) <= end_delta;
...
% % % % %             series_pred_orig(i_s,1) = series_pred(i_s,1);
...
%             if beta==0
%                 % set prediction to effective 0 value,
%                 % i.e. value midway between min_v and min_v + log range_w
%                 series_pred(i_s,1) = mean([min_v min_v+log(range_w)]);
%             elseif beta==2
%                 series_pred(i_s,1) = series(i_s,end);
%             end
            %%%
...
std_min = std(series_pred)
range_min = range(series_pred)
% std_orig = std(series_pred_orig)
