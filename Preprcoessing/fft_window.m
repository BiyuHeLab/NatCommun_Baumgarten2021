...
% [freq, power, phase, amplitude, time] = fft_window(series, fs, windowDur_inSecs,  windowOverlap_inSecs, hanning, nWindows)
%
% Do FFT analysis on a time series using sliding windows. An input time
% series is divided into a set of separate "windows" of specified temporal
% duration and overlap. Each window is detrended and demeaned (in that order) 
% prior to FFT analysis. The output returns FFT analysis separately for
% each window.
%
% inputs
% ------
...
% time      - time corresponding to the center of each window, in seconds
