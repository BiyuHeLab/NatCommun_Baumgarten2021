...
load([path_inputdata 'KprimeSummaryStruct_SSubsGAvg_TEST1000repsShuff.mat']);
Kprime_AllSub_TEST = Kprime_AllSub;
clear Kprime_AllSub
...
%         %Plot histogram to show group-level null distribution
% %         concatSubsRand_AvgChan = squeeze(concatSubsRand,2)); %Avg across channels
%         concatSubsRand_AvgChan = squeeze(concatSubsRand(:,1,:)); %Channel 1
%         concatSubsRand_AvgChanSubs = mean(concatSubsRand_AvgChan); %Avg across subs
%         numBins = 75;
%         figure;  
%         set(gcf,'units','normalized','outerposition',[0 0 1 1])
%         set(gcf,'Renderer','painters');
%         [HistOutput, BinCenter]= hist(concatSubsRand_AvgChanSubs,numBins); %compute historgram for 100 bins
%         HistOutput = HistOutput/sum(HistOutput); %normalize unit length
%         f = bar(HistOutput);  
%         xlim([0 numBins])
%         xticks(1:length(BinCenter)/10:numBins)
%         xticklabels(round(BinCenter(1:length(BinCenter)/10:end),2))
%         ylim([0 max(HistOutput)*1.5])
%         yticks(0:0.01:max(HistOutput)*1.5)
%         yticklabels(0:1:max(HistOutput)*150)
%         xlabel('Bin-centered Kprime_Shuff','Interpreter','none')
%         ylabel('relative frequency (%)') 
%         f.FaceAlpha = 0.5;
%         f.FaceColor = [1 0 0];
%         
%         %Alternative option: 1000 repetition per subject, average across same reps
%         for i_sub = 1:size(Kprime_AllSub.Shuff,1)
%             kKeepShuffles_allSubs_TEST{i_win,i_tonedur}(i_sub,:) = Kprime_AllSub_TEST.Shuff{i_sub,i_tonedur}{i_win};
%             %Appends k-prime vals per timewin/tonedur for all subs (subs/sensors/reps)
%             %Output: X k-prime values for each sensor per subject/timewin/tonedur
%         end    
%         
%         concatSubsRand_AvgChanSubs2 = squeeze(mean(kKeepShuffles_allSubs_TEST{i_win,i_tonedur},1));
%         
%         [HistOutput, BinCenter]= hist(concatSubsRand_AvgChanSubs2,BinCenter); %compute historgram for 100 bins
%         HistOutput = HistOutput/sum(HistOutput); %normalize unit length
%         hold on;
%         f = bar(HistOutput);  
%         f.FaceAlpha = 0.5;
%         f.FaceColor = [0 0 1];
%         
%         [h, p, ks2stat] = kstest2(concatSubsRand_AvgChanSubs, concatSubsRand_AvgChanSubs2);
%         
%         legend('100 repetitions per subject, 1000 iterations with random selection determined per subject', ...
%             '1000 repetitions per subject, Average across subejcts per repetition')
%         title(['Probability Distribution for Group-Level Kprime Null Distribution: KS-test: p = ' num2str(p)])
