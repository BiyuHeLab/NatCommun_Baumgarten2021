function model = libsvmtrain(training_label_vector, training_instance_matrix, libsvm_options, scale_data)
% model = libsvmtrain(training_label_vector, training_instance_matrix [, 'libsvm_options'] [, scale_data]);
% 
...
% check we're using the right svmtrain function
x = which('svmtrain');
if ~strcmp(x, '/data/gogodisk2/brian/scripts/libsvm-3.20/matlab/svmtrain.mexa64')
    disp('libsvm''s svmtrain is not being used. Instead, function svmtrain currently calls')
    disp(x)
    disp('returning...')
...
model = svmtrain(training_label_vector, training_instance_matrix_sc, libsvm_options);
