function [svm_acc, models] = libsvm_nfold_crossdecode2(group, data, nfolds, nreps, libsvm_options_train, libsvm_options_test, scale_data)
% [svm_acc, models] = libsvm_nfold(group, data, nfolds, nreps, libsvm_options_train, libsvm_options_test, scale_data)
% 
% group        - label for each trials
% data         - nTrials x nAttributes x nSamples matrix
...
% 
% svm_acc - svm accuracy for each t_train x t_test x fold x repetition
% models  - t_train x fold x repetition cell array holding the SVM models as output
%           by libsvmtrain for each training timepoint, fold, and repetition 
...
for i_fold = 1:nfolds
    ...
    % test set
    for rand_rep = 1:nreps

        for t_train = 1:nsamples
            
            % define training set for this time sample / fold / rep
            data_train_t = squeeze(data(:, :, t_train));
            ...
            % test model at t_train for all t_test
            for t_test = 1:nsamples
                
                data_test_t = squeeze(data(:, :, t_test));
                data_test   = data_test_t(ind_test, :);

                predicted_group = libsvmpredict(group_test, data_test, model, libsvm_options_test);        
            
                svm_acc(t_train, t_test, i_fold, rand_rep) = mean(predicted_group == group_test);
            end
        
            models{t_train, i_fold, rand_rep} = model;
