function NASTD_MEG_Pred_ClusterCorrPred_Group ...
    (subs, tonedur_text, ...
...
%Data output
path_output = [paths_NASTD_MEG.Current_outputdata 'PredEffect/' tonedur_title '/Group/'];
mkdir(path_output)

%% 1) Load in single subject data and format data for cluster analysis
for i_sub = 1:length(subs)
    path_input_sub = [path_input subs{i_sub} '/'];

%     load([path_input_sub subs{i_sub} '_PredEffect_' tonedur_title '.mat']); %All trials
     load([path_input_sub subs{i_sub} '_PredEffect_' tonedur_title '_lasthalf.mat']); %First or last half trial subselection
...
%% 4) Save output
% outputfile_name = ['Group_PredEffect_' tonedur_title '_ClusterCorrect_p' num2str(pval_cluster) ...
%     '_nReps' num2str(nReps_cluster) '.mat']; %All trials
outputfile_name = ['Group_PredEffect_' tonedur_title '_ClusterCorrect_p' num2str(pval_cluster) ...
    '_nReps' num2str(nReps_cluster) '_lasthalf.mat']; %First or last half trial subselection
...
%     for i_sub = 1:length(subs)
%         path_input = [paths_NASTD_MEG.Current_outputdata 'PredEffect/' tonedur_title '/' subs{i_sub} '/'];
%         load([path_input subs{i_sub} '_PredEffect_' tonedur_title '.mat']);
%         
...
%             ', uncorrected - t-test per sensor across subs)'], ['TD: ' tonedur_title '; ERF ' win_title], title_text})
%         
%         %Save topoplot
%         if save_figs    == 1
%             path_fig = ([paths_NASTD_MEG.Current_outputfig 'PredEffect/' tonedur_title '/Group/']);
%             mkdir(path_fig);
%             filename     = ['Group_PredEffect_uncorrected_' tonedur_title '_TW' w1 '-' w2 '.png'];
...
%         set(gcf,'units','normalized','outerposition',[0 0 1 1])
        ...
        title({['Group-level ' assoc_title ' p < ' num2str(pval_plotting) ...
            ', cluster-corrected; cluster sizes = ' cluster_size_text], ['TD: ' tonedur_title '; ERF ' win_title]})
        
        %Save topoplot
        if save_figs    == 1
            path_fig = ([paths_NASTD_MEG.Current_outputfig 'PredEffect/' tonedur_title '/Group/']);
            mkdir(path_fig);
            filename     = ['Group_PredEffect_ClusterCorr_' tonedur_title '_TW' w1 '-' w2 '.png'];
            figfile      = [path_fig filename];
            saveas(gcf, [figfile], 'png');
            delete(h);
        end
