function NASTD_MEG_Pred_CompPred_TrialSelection_Subs...
    (sub, tonedur_text, ...
    predictive_sequencerange, toneIndex, selected_trials,...
    plot_figs, pval_plotting, save_figs, ...
    paths_NASTD_MEG)
%Aim: Compute and save prediction effect (How is neural activity at tone 33
%modulated by the expected value of tone 34, which itself depends on the
%previous tone sequence input?)
%Note: Script performes specific trial selection by defining unique trial 
%identity based on p*34 and sequence beta level and then taking only the
%first or last half of trials with that identity as input.
...
%Determine unique trial identity for later trial subselection (first vs. last half of unique trials)
for i_trial = 1:length(data_MEG.trial)
    trial_ident(i_trial,1) =  data.stim.predID(i_trial); %p*34
    trial_ident(i_trial,2) =  data.stim.finalID(i_trial);%p34
    trial_ident(i_trial,3) =  data.stim.betaID(i_trial);%sequence beta
end

uniquepredp34 = unique(trial_ident(:,1));
uniquep34 = unique(trial_ident(:,2));
uniquebeta = unique(trial_ident(:,3));

trialindex_uniquetrials = [];
for i_uniquepredp34 = 1:length(uniquepredp34)
    for i_uniquebeta = 1:length(uniquebeta)
        trialindex_uniquetrials{i_uniquepredp34}{i_uniquebeta} = ...
            find(trial_ident(:,1) == uniquepredp34(i_uniquepredp34) & ...
            trial_ident(:,3) == uniquebeta(i_uniquebeta));
    end
end

selectedtrials_firsthalf = [];
selectedtrials_lasthalf = [];
for i_uniquepredp34 = 1:length(uniquepredp34)
    for i_uniquebeta = 1:length(uniquebeta)
                      
        selectedtrials_firsthalf = ...
            vertcat(selectedtrials_firsthalf, ...
            trialindex_uniquetrials{i_uniquepredp34}{i_uniquebeta}...
            (1:length(trialindex_uniquetrials{i_uniquepredp34}{i_uniquebeta})/2));
        selectedtrials_lasthalf = ...
            vertcat(selectedtrials_lasthalf, ...
            trialindex_uniquetrials{i_uniquepredp34}{i_uniquebeta}...
            ((length(trialindex_uniquetrials{i_uniquepredp34}{i_uniquebeta})/2)+1 : ...
            length(trialindex_uniquetrials{i_uniquepredp34}{i_uniquebeta})));        
                
    end
end

%Create filter arrays for the selected trials
Array_Alltrials = 1:length(data_MEG.trial);
f_selectedtrials_firsthalf = ismember(Array_Alltrials, sort(selectedtrials_firsthalf)');
f_selectedtrials_lasthalf = ismember(Array_Alltrials, sort(selectedtrials_lasthalf)');

%Select specific trials
numoftrials = num2str(length(data.trialNum));

if strcmp (selected_trials, 'firsthalf')
    dat_fields  = fieldnames(data);
    for i = 1:length(dat_fields)
        if eval(['length(data.' dat_fields{i} ') ==' numoftrials])
            eval(['data.' dat_fields{i} ' = data.' dat_fields{i} '(f_selectedtrials_firsthalf);']);
        end
    end
    ...
    for i = 1:length(stim_fields)
        if eval(['length(data.stim.' stim_fields{i} ') ==' numoftrials])
            eval(['data.stim.' stim_fields{i} ' = data.stim.' stim_fields{i} '(f_selectedtrials_firsthalf);']);
        end
    end
    ...
    for i = 1:length(MEG_fields)
        if eval(['length(data_MEG.' MEG_fields{i} ') ==' numoftrials])
            eval(['data_MEG.' MEG_fields{i} ' = data_MEG.' MEG_fields{i} '(f_selectedtrials_firsthalf);']);
        end
    end
    
elseif strcmp (selected_trials, 'lasthalf')
    
    dat_fields  = fieldnames(data);
    for i = 1:length(dat_fields)
        if eval(['length(data.' dat_fields{i} ') ==' numoftrials])
            eval(['data.' dat_fields{i} ' = data.' dat_fields{i} '(f_selectedtrials_lasthalf);']);
        end
    end
    ...
    for i = 1:length(stim_fields)
        if eval(['length(data.stim.' stim_fields{i} ') ==' numoftrials])
            eval(['data.stim.' stim_fields{i} ' = data.stim.' stim_fields{i} '(f_selectedtrials_lasthalf);']);
        end
    end
    ...
    for i = 1:length(MEG_fields)
        if eval(['length(data_MEG.' MEG_fields{i} ') ==' numoftrials])
            eval(['data_MEG.' MEG_fields{i} ' = data_MEG.' MEG_fields{i} '(f_selectedtrials_lasthalf);']);
        end
    end
end
...
%% 7) Save variables
savefile = [path_save sub '_PredEffect_' tonedur_title  '_' selected_trials '.mat'];
...
if plot_figs == 1
    ...
    for i_k = 1:length(predictive_sequencerange)
        ...
        for i_win = 1:size(windows,1)
            ...
            title_text = ['evaluated at tone #' num2str(toneIndex) ', prediction using k = ' ...
                num2str(predictive_sequencerange(i_k)) ' tones; selected trials: ' selected_trials];
            ...
            %Save topoplot
            if save_figs    == 1
                path_fig = ([paths_NASTD_MEG.Current_outputfig 'PredEffect_uncorrected/' tonedur_title '/' sub '/' selected_trials '/']);
                mkdir(path_fig);
                filename     = [sub '_PredEffect_uncorrected_' tonedur_title '0_TW' w1 '-' w2 '_' selected_trials '.png'];
